...
class CustomerConnectionsComponent extends Component
{
    ...
    public array $selectedRowCheckbox = [];
    public array $unSelectedRowCheckbox = [];
...
//        'loadMoreForConnectionsReport',
    ...
    {
        $this->emit('datePickerHydrate');
        $this->emit('loadContentPage');
        $this->emit('initDragAndDropElem');
    }

    public function mount($customer = null, $page = null)
    {
        $this->pageNumber = 1;
        if (session()->has('todo_view_action_type')) {
            if (session('todo_view_action_type') == 'tariff-overlapping') {
                $this->selectedStatus = 6;
            } elseif (session('todo_view_action_type') == 'time-overlapping') {
                $this->selectedStatus = 5;
            } elseif (session('todo_view_action_type') == 'recover-pending-connection') {
    ...
    public function render()
    {
        ...
        return view('livewire.tenant.customer-connections-component', [
    ...
    public function getTickedCheckboxes()
    {
        dd('$this->selectedRowCheckbox =>', $this->selectedRowCheckbox);
    }
    ...
    public function toggleCheckBox($connId)
    {
        if (($key = array_search($connId, $this->selectedRowCheckbox)) !== false) {
            unset($this->selectedRowCheckbox[$key]);
        } else {
            $this->selectedRowCheckbox = array_push($this->selectedRowCheckbox, $connId);
        }
    }

    public function updatedSelectedRowCheckbox($value)
    {
        dd($this->selectedRowCheckbox,$value);
        $this->selectedRowCheckbox = is_array($this->selectedRowCheckbox) && count($this->selectedRowCheckbox)
            ? array_unique($this->selectedRowCheckbox)
            : [];
...
/*    public function checkIfCheckboxIsSelected($value)
    {
        dd('checkIfCheckboxIsSelected -> $value',$value);
...
//                ->where('userid',auth()->id())
    ...
    public function filterBySelectedInputs()
    {
        ...
        if ($this->customer) {
            ...
            $this->emit('connectionReportStore');
        }
    }
    ...
    public function calculateTotals()
    {
        ...
        if ($this->start_date || $this->end_date) {
            if ($this->customer) {
                ...
                group($this->customer->bdgogid)
                    ...
                    ->chunk(200, function ($connections) use (&$totalDuration, &$totalAmount, &$totalUnits) {
                        foreach ($connections as $connection) {
                            $results = $connection->calculateIntervalWithPriceIndividual();
                            $totalDuration += $results['with_interval'];
                            $totalAmount += $results['with_interval_price'];
                            $totalUnits += $connection->calculateUnit();
                        }
                    });
            }
        } elseif (!$this->start_date && !$this->end_date) {
        ...
        return [
            ...
            'totalDuration' => \App\Helpers\Helper::formatHoursAndMinutes($totalDuration * 60,'%02d:%02d',true),
    ...
    public function generatePDF($action)
    {
        ...
        if (count($connections)) {
            ...
            $appliedTariffsArr = [];
            $appliedTariffs = new Collection();
            $charged->map(function ($chargedConn) use ($appliedTariffs, &$appliedTariffsArr) {
                if (!empty($chargedConn->tariff->id) && !in_array($chargedConn->tariff->id, $appliedTariffsArr)) {
                    array_push($appliedTariffsArr, $chargedConn->tariff->id);
                    $appliedTariffs->push($chargedConn->tariff);
                    foreach ($appliedTariffs as $appliedTariff) {
                        if ($appliedTariff->id === $chargedConn->tariff->id) {
                            $appliedTariff->total_unit = $chargedConn->calculateUnit() ?? 0;
                            $appliedTariff->total_price = $chargedConn->price ? floatval($chargedConn->price) : 0;
                        }
                    }
                } else {
                    foreach ($appliedTariffs as $appliedTariff) {
                        if ($appliedTariff->id === $chargedConn->tariff->id) {
                            $appliedTariff->total_unit = $chargedConn->calculateUnit() ? $chargedConn->calculateUnit() + $appliedTariff->total_unit : $appliedTariff->total_unit;
                            $appliedTariff->total_price = $chargedConn->price ? floatval($chargedConn->price) + $appliedTariff->total_price : $appliedTariff->total_price;
                        }
                    }
                }
            });
...
//            return redirect(url($url));
    ...
    public function startChronos()
    {
        ...
        \Log::info('appCustomerConnectionsComponent : ' . session()->has('openTimerComponent'));
//        $this->emitTo('tenant.partials.timer-nav-component','startChronos');
    ...
    /*public function checkboxSelected($value)
    {
        ...
        if (ConnectionReport::withTrashed()->find($value)->trashed()) {
            $pos = array_search($value, $this->unSelectedRowCheckbox);
            if ($pos) {
                unset($this->unSelectedRowCheckbox[$pos]);
            } else {
                $this->unSelectedRowCheckbox = array_merge($this->unSelectedRowCheckbox, [$value => $value]);
            }
        } else {
            if ($pos = array_search($value, $this->selectedRowCheckbox)) {
                unset($this->selectedRowCheckbox[$pos]);
            } else $this->selectedRowCheckbox = array_merge($this->selectedRowCheckbox, [$value => $value]);
        }
        if ($this->isLoadMore && $this->hasMorePage) {
            $this->emitTo('tenant.customer-connections-load-more-component', 'loadMoreUpdatePage');
            $this->dispatchBrowserEvent('loadMoreConnectionOnRefresh');
        }
    }*/
