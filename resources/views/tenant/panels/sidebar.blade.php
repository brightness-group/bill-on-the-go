...
<div class="main-menu menu-fixed @if($configData['theme'] === 'light') {{"menu-light"}} @else {{'menu-dark'}} @endif menu-accordion menu-shadow" data-scroll-to-active="true">
      ...
      <li class="nav-item mr-auto">
          ...
          <div class="brand-logo">
            <img src="{{asset('images/logo/billonthego-logo.png')}}" class="logo" style="width:170px;height:30px;" alt="">
          </div>
          </a>
      </li>
      </ul>
      ...
      <div class="main-menu-content">
          ...
          @foreach ($menuData[0]->menu as $menu)
              @if(isset($menu->navheader))
                  @if($menu->navheader == 'tenant-name')
                        <li class="navigation-header mb-2"><span style="font-size: 14px" class="box-shadow-6">{{ $tenant }}</span></li>
                  @elseif($menu->navheader == 'Others')
                        <li class="navigation-header mt-5 mb-2"><span>{{ __('locale.'.$menu->navheader) }}</span></li>
                  @else
                    <li class="navigation-header mt-2 mb-2"><span>{{ __('locale.'.$menu->navheader) }}</span></li>
                  @endif
              @else
                  <li class="nav-item {{(request()->is($menu->url.'*')) || (request()->is('customer*') && $menu->name == "Customers") ? 'active' : '' }} mb-1">
                      ...
                      @if(isset($menu->submenu))
                          @include('tenant.panels.sidebar-submenu',['menu' => $menu->submenu])
                      @endif
                  </li>
              @endif
          @endforeach
