          ...
          <div class="card disable-rounded-right mb-0 p-2">
            <div class="card-header pb-1">
              <div class="card-title">
                <h4 class="text-center mb-2">{{ __('locale.Forgot Password') }}?</h4>
              </div>
            </div>
            <div class="form-group d-flex justify-content-between align-items-center mb-2">
              <div class="text-left">
                <div class="ml-3 ml-md-2 mr-1">
                  <a href="{{asset('login')}}"  class="card-link btn btn-outline-primary text-nowrap">{{ __('locale.Sign in') }}</a>
                </div>
              </div>
              <div class="mr-3">
                <a href="{{asset('register')}}" class="card-link btn btn-outline-primary text-nowrap">{{ __('locale.Sign up') }}</a>
              </div>
            </div>
              ...
              <div class="card-body">
                ...
                <form class="mb-2" method="POST" action="{{ route('password.email') }}">
                  ...
                  <div class="form-group mb-2">
                    <label class="text-bold-600" for="email">{{ __('locale.Email') }}</label>
                  ...
                  <button type="submit" class="btn btn-primary glow position-relative w-100">{{ __('locale.SEND PASSWORD') }}
                ...
                <div class="text-center mb-2">
                  <a href="{{asset('login')}}">
                    <small class="text-muted">{{ __('locale.I remembered my password') }}</small>
                  </a>
                </div>
                <div class="divider mb-2">
                  <div class="divider-text">{{ __('locale.Or Sign in as') }}</div>
                </div>
